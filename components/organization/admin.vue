...
const ao_management_data = computed(
	() => useAoManagementStore().$state.ao_management
);
...
const handleArchiveOrganization = async (id: number) => {
	try {
		await archiveOrganization({ organizationId: id, supabase: supabase });
	} catch (error) {
		console.log(error);
...
<template>
	<div class="flex flex-col p-8 gap-4">
		...
		<Transition mode="out-in" name="fade">
			...
			<div
				v-else-if="!pending && ao_management_data?.length !== 0"
							...
							<DropdownMenu>
								...
								<DropdownMenuContent>
									...
									<DropdownMenuItem
										:onclick="
											() =>
												handleArchiveOrganization(organization.organization_id)
										"
										>Archive
									</DropdownMenuItem>
								</DropdownMenuContent>
			...
			<div
				...
				class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 transition-all duration-1000"
			>
				<NuxtLink to="/organizations/create">
					<Card
						class="flex flex-row gap-2 border sm:flex-row justify-center items-center col-span-1 h-auto sm:h-52 p-8 transition-all duration-200 hover:border-white/50 text-white/75 hover:text-white/100"
					>
						<PlusCircleIcon />
