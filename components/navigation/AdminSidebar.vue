<template>
	<div class="flex flex-col px-7 py-8 gap-1">
		<NuxtLink v-for="item in navItems" :key="item.name" :to="item.path">
			<div class="pointer-events-none" v-if="item.name === 'Organizations'">
				<Separator class="my-2 pointer-events-none" />
			</div>
			<div class="flex flex-row items-center px-4 py-2 rounded-sm hover:bg-primary/10 gap-4">
			...
			<CollapsibleContent>
				<div class="flex flex-col select-none">
					<NuxtLink to="/settings/general"
						class="flex flex-row items-center pl-14 px-4 py-2 rounded-sm hover:text-current/50 gap-4">
						General</NuxtLink>
