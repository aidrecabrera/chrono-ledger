...
const { data: mo_association_data, refresh: refresh_mo_association_data, pending }: { data: Ref<MoAssociation[] | null>, refresh: () => void, pending: any } = await useAsyncData('get_mo_association_data', async () => {
...
<template>
  <div class="p-8">
    <Transition mode="out-in" name="fade">
      <div v-if="!pending" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 transition-all duration-1000">
        <Card v-for="organization in mo_data" :key="organization.organization_id"
      ...
      <div v-else>
        <div class="flex flex-col min-w-full gap-2 pt-8 justify-center items-center">
