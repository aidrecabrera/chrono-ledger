...
// * Realtime updates for mo_association data
let moAssociationMoAssociationChannel: RealtimeChannel | null = null;
moAssociationMoAssociationChannel = supabase.channel('public:mo_association')
  .on('postgres_changes',
    { event: '*', schema: 'public', table: 'mo_association' },
    (payload) => {
      refresh_mo_association_data();
    }
  )
moAssociationMoAssociationChannel.subscribe();
// * Unsubscribing when user is not on the page
if (!useAdminInformationStore().$state.information) {
  moAssociationMoAssociationChannel.unsubscribe();
}

useMoAssociationStore().$subscribe((mutation) => {
  if ('payload' in mutation && 'mo_association' in mutation.payload) refresh_mo_association_data()
})
...
const { data: mo_association_data, refresh: refresh_mo_association_data, pending }: { data: Ref<MoAssociation[] | null>, refresh: () => void, pending: any } = await useAsyncData('mo_association_data_get', async () => {
...
// * Watch for changes in mo_association and pending state
watch([mo_association_data, pending], ([newMoAssociationData, newPending]) => {
  useMoAssociationStore().$patch({
    mo_association: newMoAssociationData,
    pending: newPending
  });
});
...
<template>
  <div class="p-8">
    <Transition mode="out-in" name="fade">
      <div v-if="mo_pending && mo_data?.length === 0">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 transition-all duration-1000">
        <Card v-for="organization in mo_association_data" :key="organization.organization_id"
